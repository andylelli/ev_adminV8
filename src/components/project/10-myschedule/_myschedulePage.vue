<template>
	<f7-page stacked name="myschedule">
		<!-- Nav bar-->
		<f7-navbar>
		...
		<segment header="General">
			<general-settings page="myschedule" :projectid="projectid"></general-settings>
		</segment>
	</f7-page>
</template>
			...
			this.$watch(
				...
				function (newValue, oldValue) {
					if (this.getSchedule) {
						...
						var item = {};
						item.table = "myschedule";
						item.json = this.getMYschedule;

						store.dispatch("updateItemDB", item);

						localStorage.admin_update_myschedule_time = unixtime;
					}
				},
				{ deep: true }
			);
