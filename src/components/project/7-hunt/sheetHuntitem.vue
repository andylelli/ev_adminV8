  ...
  >
    <f7-page-content>
      ...
      <div v-if="getHuntitem">
        <image-load table="huntitem" :id="getHuntitem.huntitem_id"></image-load>
        <text-block
          table="huntitem"
...
export default {
  ...
  mounted() {
    ...
    this.eventBus.on("sheet-huntitem", (id) => {
      ...
      if (vue.eventBus.eventsListeners["sheet-huntitem"].length > 1) {
        vue.eventBus.eventsListeners["sheet-huntitem"].splice(1);
      }
    });
  },
};
