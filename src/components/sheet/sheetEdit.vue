<template>
    <f7-sheet id="sheet-edit" style="height: auto; --f7-sheet-bg-color: #fff" close-by-backdrop-click backdrop @sheet:closed="clearForm()">
        <f7-page-content>
            <f7-block class="padding-top">
                <f7-block-title class="padding-bottom">
                    {{ this.titleString }}</f7-block-title>
                <f7-list form id="aaa">
                    <f7-list-input :key="this.fieldname" :id="fieldId(this.fieldname)" :type="this.fieldtype" step="0.01" :placeholder="this.title" :value="decode(this.value)" @input="update($event)" validate required></f7-list-input>
                </f7-list>
                <div style="padding-left: 25px; padding-right: 25px; height: 50px">
                    <div v-show="this.allowChange === true">
                        <general-button class="margin-top margin-bottom" @generalButtonAction="submit()" label="UPDATE" width="200" type="fill"></general-button>
                    </div>
                </div>
            </f7-block>
        </f7-page-content>
    ...
    methods: {
        ...
        submit() {
            if (this.isValid() === true) {
                ...
                this.changed = false;
                f7.sheet.close("#sheet-edit", true);
            }
        },
    },
    mounted() {
        var vue = this;
        this.eventBus.on("edit-item", (json) => {
            vue.id = json.id;
